...
class gdsih_core_db extends d4p_wpdb_core {
    ...
    public function csp_report($data = array()) {
        $defaults = array('reported' => $this->datetime(), 'ip' => gdsih()->ip(),
            'violated_directive' => '', 'effective_directive' => '', 'document_uri' => '',
            'blocked_uri' => '', 'referrer' => '', 'original_policy' => '', 'user_agent' => '');
    ...
    public function xxp_report($data = array()) {
        $defaults = array('reported' => $this->datetime(), 'ip' => gdsih()->ip(),
            'request_url' => '', 'request_body' => '', 'user_agent' => '');
        $data = wp_parse_args($data, $defaults);
